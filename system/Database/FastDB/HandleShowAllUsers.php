...
class HandleShowAllUsers
{
	...
	public function handle()
	{
        ...
        $pathInServer = Store::replace(['%server' => $this->server], $this->path['patchUsers']);

        $usersData = [];
        
        foreach(Store::scan($pathInServer) as $userDir)
        {

            $pathFileUser = $pathInServer.'/'.$userDir.'/user-'.$userDir.'.json';

            $data = Store::uncompress(Store::getApi($pathFileUser));
            $usersData[] = Response::jsonToArray($data)[0];
